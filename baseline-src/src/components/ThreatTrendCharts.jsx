...
export default function ThreatTrendCharts({ threats }) {
  ...
  const critical30 = data.reduce((s, d) => s + d.Critical, 0);
  const peak = data.reduce((max, d) => (d.total > max.total ? d : max), data[0] || { day: "—", total: 0 });

  return (
    <div className="rounded-xl border border-border bg-card p-6 mb-6">
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <h3 className="font-semibold">30-Day Threat Trends</h3>
        <div className="flex items-center gap-4 text-xs text-muted-foreground">
          <span>
            <span className="font-semibold text-foreground">{total30}</span> threats
          </span>
          <span>
            <span className="font-semibold text-red-500">{critical30}</span> critical
          </span>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={data} margin={{ left: -12, right: 8, top: 4 }}>
              ...
              <XAxis dataKey="day" tickLine={false} axisLine={false} className="text-xs" interval={5} />
