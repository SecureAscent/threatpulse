...
import AttckTechniques from "@/components/AttckTechniques";
      ...
      <div className="rounded-xl border border-border bg-card p-6 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-1 mt-4 text-xs text-muted-foreground">
          ...
          {threat.epss_score != null && (
            <span className="font-mono text-primary" title="Exploit Prediction Scoring System — likelihood of exploitation in next 30 days">
              EPSS {(threat.epss_score * 100).toFixed(1)}%{threat.epss_percentile != null ? ` · pct ${(threat.epss_percentile * 100).toFixed(0)}` : ""}
            </span>
          )}
        </div>
      </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between gap-2">
                ...
                <dd className="text-right font-mono">{threat.epss_score != null ? `${(threat.epss_score * 100).toFixed(1)}%` : "—"}</dd>
              </div>
              <div className="flex justify-between gap-2">
                <dt className="text-muted-foreground">EPSS pct</dt>
                <dd className="text-right font-mono">{threat.epss_percentile != null ? `${(threat.epss_percentile * 100).toFixed(0)}` : "—"}</dd>
              </div>
              {threat.source_url && (
