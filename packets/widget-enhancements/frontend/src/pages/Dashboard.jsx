...
function StatCard({ icon: Icon, label, value, color, bg, to }) {
  ...
  if (to) {
    return (
      <Link to={to} className="block rounded-xl border border-border bg-card p-5 hover:border-primary/40 hover:shadow-sm transition-all">
        {content}
      </Link>
    );
  }
...
export default function Dashboard() {
  ...
  const { data: threats = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Threat.filter({ archived: { $ne: true } }, "-created_date", 500),
  });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon={Shield} label="Total Threats" value={total} color="text-primary" bg="bg-primary/10" to="/threats" />
        <StatCard icon={AlertTriangle} label="Critical" value={critical} color="text-red-500" bg="bg-red-500/10" to="/threats" />
        <StatCard icon={Zap} label="High Severity" value={high} color="text-orange-500" bg="bg-orange-500/10" to="/threats" />
        <StatCard icon={TrendingUp} label="Today" value={today} color="text-emerald-500" bg="bg-emerald-500/10" to="/threats" />
      </div>
              ...
              return (
                <div key={s.name}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-muted-foreground">{s.count} <span className="text-xs opacity-60">({pct.toFixed(0)}%)</span></span>
                  </div>
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/threats" className="text-xs text-primary font-medium hover:underline flex items-center gap-1">
          ...
          <div className="space-y-1">
            {recent.map((t) => (
              <Link
                key={t.id}
                to={`/threats/${t.id}`}
                className="flex items-center gap-3 py-2.5 border-b border-border last:border-0 -mx-2 px-2 rounded-lg hover:bg-accent/30 transition-colors"
              >
